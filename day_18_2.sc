...
object Main {
    ...
    def computeAnswer(numbers: List[Number]): Int = {
        ...
        for (i <- 0 to numbers.length - 1) {
            for (j <- 0 to numbers.length - 1) {
                if (i != j) {
                    val additionMagnitude = magnitude(add(numbers(i), numbers(j)))
                    if (additionMagnitude > maxMagnitude) {
                        maxMagnitude = additionMagnitude
                    }
                }
            }
        }
    ...
    def main(args: Array[String]) = {
        val source = Source.fromFile("input/input_18_2.txt")
