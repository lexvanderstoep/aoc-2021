...
NSMutableDictionary<NSString*, NSNumber*>* parsePolymerTemplate(NSString* filename, NSString** firstPair, NSString** lastPair) {
    ...
    for (int i = 0; i < [templatePolymer length] - 1; i++) {
        ...
        dictionary[pair] = [NSNumber numberWithLong:([count longValue] + 1)];
    }
...
NSMutableDictionary<NSString*, NSNumber*>* applyStep(NSMutableDictionary<NSString*, NSNumber*>* polymer, NSString** firstPair, NSString** lastPair, NSMutableDictionary<NSString*, NSString*>* insertionRules) {
    ...
    for(id pair in polymer) {
        ...
        newPolymer[newFirstPair] = [NSNumber numberWithLong:([newPolymer[newFirstPair] longValue] + [count longValue])];
        newPolymer[newSecondPair] = [NSNumber numberWithLong:([newPolymer[newSecondPair] longValue] + [count longValue])];
    }
...
NSMutableDictionary<NSString*, NSNumber*>* countOccurences(NSMutableDictionary<NSString*, NSNumber*>* polymer, NSString* firstPair, NSString* lastPair) {
    ...
    for (id pair in polymer) {
        ...
        occurences[firstCharacter] = [NSNumber numberWithLong:([occurences[firstCharacter] longValue] + [count longValue])];
        occurences[secondCharacter] = [NSNumber numberWithLong:([occurences[secondCharacter] longValue] + [count longValue])];
    }
    ...
    occurences[firstCharacter] = [NSNumber numberWithLong: ([occurences[firstCharacter] longValue] + 1)];
    occurences[lastCharacter] = [NSNumber numberWithLong: ([occurences[lastCharacter] longValue] + 1)];
    ...
    for (id character in occurences) {
        ...
        singleOccurences[character] = [NSNumber numberWithLong: [occurences[character] longValue] / 2];
    }
...
int main(int argc, const char * argv[]) {
    @autoreleasepool {
        ...
        NSString* firstPair;
        NSString* lastPair;
        id polymer = parsePolymerTemplate(@"input/input_14_2.txt", &firstPair, &lastPair);
        id insertionRules = parseInsertionRules(@"input/input_14_2.txt");
